...
def salva():
    ...
    print(f"\nOs arquivos de login do motorista {idn}, foram salvos em {caminho_destino}")
...
def statusmotorista(ticket):
    ...
    if response.status_code == 200:
        ...
        data = response.json().get('data', {}).get('abilities', {}).get('driver/dIndexInfo', {}).get('data', {})

        if data:
            if data.get('driver_status_info', {}).get('conectar_status', ''):
                if data.get('driver_status_info', {}).get('conectar_status', '') == 1:
                    return f"\033[92mOnline \033[0m"
                if data.get('driver_status_info', {}).get('conectar_status', '') == 2:
                    expire_time_timestamp = data.get('driver_status_info', {}).get('expire_time', '')
...
def saldomotorista(ticket):
    ...
    response = requests.post(url, headers=headers, json=request_body)
    #print(response.json().get('data', {}).get('abilities', {}).get('expo/dIncomeHome', {}).get('data', {}).get('driver_logic', {}).get('balance', {}).get('text', ''))
...
if acao.upper() == 'R':
    bacao = input("Deseja visualizar dados salvos ou atualizar  (V/A)")
    ...
    if bacao.upper() == 'A':
        ...
        if motoristas_ordenados:                    
            login = input(f"\n\nO motorista da vez é {motoristas_ordenados[0]['id']}, deseja carregar arquivos do login? (S/N)")
            if login == "S":
